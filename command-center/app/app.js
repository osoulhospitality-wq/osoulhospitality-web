(() => {
  "use strict";
  const KEY = "osoul-command-center-v10";
  ...
  const blank = () => ({version:10,suppliers:[],products:[],quotes:[],contracts:[],decisions:[],updatedAt:new Date().toISOString()});
  let state;
  try { state = JSON.parse(localStorage.getItem(KEY)) || blank(); } catch { state = blank(); }

  const seed = () => {
    const s1=uid(),s2=uid(),s3=uid(),p1=uid(),p2=uid();
    state={version:10,updatedAt:new Date().toISOString(),
      suppliers:[
  ...
  const save = message => { state.updatedAt=new Date().toISOString();localStorage.setItem(KEY,JSON.stringify(state));render();if(message) toast(message); };
  ...
  function render(){
    renderOptions(); renderDashboard(); renderSuppliers(); renderProducts(); renderQuotes(); renderContracts(); renderDecisions();
  }
  ...
  function exportData(){
    const blob=new Blob([JSON.stringify(state,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`osoul-command-center-backup-${today()}.json`;a.click();URL.revokeObjectURL(a.href);toast("تم تنزيل النسخة الاحتياطية");
  }
  function handleForm(form){
    ...
    form.closest("dialog").close();form.reset();save("تم حفظ السجل");
  }
  document.addEventListener("click",e=>{
    ...
    if(action==="reset"&&confirm("حذف جميع بيانات هذه النسخة من الجهاز؟")){state=blank();save("تم مسح البيانات");}
    const del=e.target.closest("[data-delete]");if(del&&confirm("حذف هذا السجل؟")){state[del.dataset.delete]=state[del.dataset.delete].filter(x=>x.id!==del.dataset.id);save("تم حذف السجل");}
  });
  ...
  $("#restoreFile").addEventListener("change",async e=>{try{const parsed=JSON.parse(await e.target.files[0].text());if(!parsed.suppliers||!parsed.products)throw Error();state=parsed;save("تمت استعادة النسخة");}catch{toast("ملف النسخة غير صالح");}e.target.value="";});
  render();
})();
