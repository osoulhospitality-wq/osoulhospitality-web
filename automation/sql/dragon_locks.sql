...
-- Supabase exposes public-schema tables through PostgREST. These tables are
-- automation-internal, so browser/anon/authenticated clients must never access
-- them. n8n writes through a service-role credential kept in its credential
-- vault; service_role bypasses RLS by design.
ALTER TABLE dragon_locks ENABLE ROW LEVEL SECURITY;
ALTER TABLE dragon_events_log ENABLE ROW LEVEL SECURITY;

REVOKE ALL ON TABLE dragon_locks FROM anon, authenticated;
REVOKE ALL ON TABLE dragon_events_log FROM anon, authenticated;
REVOKE ALL ON SEQUENCE dragon_locks_id_seq FROM anon, authenticated;
REVOKE ALL ON SEQUENCE dragon_events_log_id_seq FROM anon, authenticated;

GRANT SELECT, INSERT, UPDATE ON TABLE dragon_locks TO service_role;
GRANT SELECT, INSERT ON TABLE dragon_events_log TO service_role;
GRANT USAGE, SELECT ON SEQUENCE dragon_locks_id_seq TO service_role;
GRANT USAGE, SELECT ON SEQUENCE dragon_events_log_id_seq TO service_role;
...
-- PostgREST must include:
--   ?on_conflict=repository,issue_number,command_version
-- together with:
